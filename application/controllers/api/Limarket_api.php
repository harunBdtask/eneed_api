...
class Limarket_api extends CI_Controller
{
    public function __construct()
    {
        ...
        //limarket_api_model load
        $this->load->model('api/limarket_api_model');
    }
    ...
    */
    ...
    |   customer login
    |   GET method 
    |   route = api/limarket_api/customer_login
    |-------------------------------
    */
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($error != '') {
                    ...
                    $com_info = $this->limarket_api_model->retrieve_company_editdata();
                    if (!empty($com_info)) {
    ...
    public function promo_cat_list()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                JSONSuccessOutput($data, check_api_token());
            }
        }
    }
    /*
    ...
    */
    public function promotion_product()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db->select('a.product_id,a.seller_id,a.category_id,a.price,a.offer_price,a.vat,a.unit,a.quantity,a.meta_title,a.thumb_image_url,
                ...
                $this->db->join('product_title c', "a.product_id = c.product_id AND c.lang_id='$lang_id' ", 'left');
                ...
                $query = $this->db->get();
                $promotion_product = $query->result_array();
                //response data list
                $data = array(
                    'base_url' => base_url(),
    ...
    /*
    |-------------------------------
    |   promotion_brand Load Here
    |   route = api/limarket_api/promotion_brand
    |-------------------------------
    */
    public function promotion_brand()
    {
        //api key checking
        if (checkAuth(check_api_key())) {
            $this->form_validation->set_rules('category_id', 'Category ID', 'trim|xss_clean');
            if ($this->form_validation->run() == FALSE) {
                ...
                //load from model file
                $this->db->select('b.brand_id as brand_id, IF(b.brand_name is null, "", b.brand_name) as brand_name, IF(b.brand_image is null, "", b.brand_image) as brand_image, IF(b.website is null, "", b.website) as website');
                $this->db->from('product_information a');
                $this->db->join('brand b', 'a.brand_id = b.brand_id', 'left');
                $this->db->join('product_category c', 'a.category_id = c.category_id', 'left');
                $this->db->where('b.brand_id is not null');
                $this->db->where('c.on_promotion', 1);
                ...
                $query = $this->db->get();
                if ($query->num_rows() > 0) {
                    $promotion_brand = $query->result();
                } else {
                    $promotion_brand = array();
                }
                //response data list
                $data = array(
                    'base_url' => base_url(),
    ...
    /*
    |-------------------------------
    |   promotion_seller Load Here
    |   route = api/limarket_api/promotion_seller
    |-------------------------------
    */
    public function promotion_seller()
    {
        //api key checking
        if (checkAuth(check_api_key())) {
            $this->form_validation->set_rules('category_id', 'Category ID', 'trim|xss_clean');
            if ($this->form_validation->run() == FALSE) {
                ...
                $category_id = filter_input_data($this->input->post('category_id', TRUE));
                //load from model file
                $this->db->select('b.seller_id as seller_id, IF(b.image is null, "", b.image) as image, IF(b.business_name is null, "", b.business_name) as business_name, IF(b.seller_store_name is null, "", b.seller_store_name) as seller_store_name');
                $this->db->from('product_information a');
                $this->db->join('seller_information b', 'a.seller_id = b.seller_id', 'left');
                $this->db->join('product_category c', 'a.category_id = c.category_id', 'left');
                $this->db->where('c.on_promotion', 1);
                if (!empty($category_id)) {
                    $this->db->where('c.category_id', $category_id);
                ...
                $query = $this->db->get();
                if ($query->num_rows() > 0) {
                    $promotion_seller = $query->result();
                } else {
                    $promotion_seller = array();
                }
                //response data list
                $data = array(
                    'base_url' => base_url(),
                    'promotion_seller' => $promotion_seller,
                );
                JSONSuccessOutput($data, check_api_token());
            }
        }
    ...
    |   route = api/limarket_api/slider_list
    |-------------------------------
    */
    public function slider_list()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                //load from model file
                $slider_list = $this->limarket_api_model->slider_list($per_page, $page, $status);
                //response data list
                $data = array(
                    'base_url' => base_url(),
    ...
    |   route = api/limarket_api/category_list
    |-------------------------------
    */
    public function category_list()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            $this->form_validation->set_rules('per_page', 'Data Limit Per Page', 'trim|is_natural_no_zero|numeric|xss_clean');
            $this->form_validation->set_rules('page', 'Page Number', 'trim|is_natural_no_zero|numeric|xss_clean');
            $this->form_validation->set_rules('featured', 'Featured', 'trim|numeric|xss_clean');
            $this->form_validation->set_rules('top_menu', 'Top Menu', 'trim|numeric|xss_clean');
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($errors['featured'])) {
                    $errors_data = $errors['featured'];
                }
                if (!empty($errors['top_menu'])) {
                    $errors_data = $errors['top_menu'];
                }
                JSONErrorOutput($errors_data);
            } else {
                $user_lang = filter_input_data($this->input->post('language', TRUE));
                $per_page = filter_input_data($this->input->post('per_page', TRUE));
                $page = filter_input_data($this->input->post('page', TRUE));
                $featured = filter_input_data($this->input->post('featured', TRUE)); // active = 1, inactive = 0
                $top_menu = filter_input_data($this->input->post('top_menu', TRUE)); // active = 1, inactive = 0
                $cat_type = filter_input_data($this->input->post('cat_type', TRUE));// 1=parent,2=sub caregory  
                $category_id = filter_input_data($this->input->post('category_id', TRUE));
                $parent_category_id = filter_input_data($this->input->post('parent_category_id', TRUE));
                $category_name = filter_input_data($this->input->post('category_name', TRUE));
                $menu_pos = filter_input_data($this->input->post('menu_pos', TRUE));
                $home_page = filter_input_data($this->input->post('home_page', TRUE));
                $on_promotion = filter_input_data($this->input->post('on_promotion', TRUE));
                $promo_date_start = filter_input_data($this->input->post('promo_date_start', TRUE));
                $promo_date_end = filter_input_data($this->input->post('promo_date_end', TRUE));
                $on_promo_image = filter_input_data($this->input->post('on_promo_image', TRUE));
                $home_promo = filter_input_data($this->input->post('home_promo', TRUE));
                if (empty($user_lang)) {
                    $lang_id = 'english';
                }else{
                    $lang_id = $user_lang;
                ...
                $this->db->select('a.category_id, a.category_name, a.cat_image, a.cat_favicon, IF(a.meta_title is null, "", a.meta_title) as meta_title');
                $this->db->from('product_category a');
                $this->db->where('a.status', 1);
                
                if ($featured != '' && $featured != null) {
                    $this->db->where('a.featured', $featured);
                }
                if ($top_menu != '' && $top_menu != null) {
                    $this->db->where('a.top_menu', $top_menu);
                }
                if ($cat_type != '' && $cat_type != null) {
                    $this->db->where('a.cat_type', $cat_type);
                }
                if (!empty($category_id)) {
                    $this->db->where('a.category_id', $category_id);
                }
                if (!empty($parent_category_id)) {
                    $this->db->where('a.parent_category_id', $parent_category_id);
                }
    ...
    */
    public function product_list()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                    'base_url' => base_url(),
                    'product_count' => count($response_data),
    ...
    /*
    |-------------------------------
    |   best_sales_product_list
    |   route = api/limarket_api/best_sales_product_list
    |-------------------------------
    */
    public function best_sales_product_list()
    {
        //api key checking
        if (checkAuth(check_api_key())) {
            $this->form_validation->set_rules('language', 'System Language', 'trim|xss_clean');
            ...
            ');
            $this->db->from('product_information a');
            $this->db->join('product_information_translation t', "a.product_id = t.product_id", 'left');
            $this->db->join('product_category b', 'a.category_id = b.category_id', 'left');
            ...
            $this->db->group_by('a.product_info_id');
            $this->db->limit($per_page, $page);
            $query = $this->db->get();
            $response_data = $query->result_array();
                $data = array(
                    'base_url' => base_url(),
                    'best_sales_product_list' => $response_data,
                );
                JSONSuccessOutput($data, check_api_token());
            }
        }
    ...
    |-------------------------------
    |   random_product_list
    |   route = api/limarket_api/random_product_list
    |-------------------------------
    */
    public function random_product_list()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                    'base_url' => base_url(),
                    'random_product_list' => $response_data,
                );
                JSONSuccessOutput($data, check_api_token());
            }
        }
    ...
    public function product_description()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $description_type = filter_input_data($this->input->post('description_type', TRUE)); //1=description, 2=specification   
                //load from model file
                $this->db->select('*');
    ...
    public function product_details()
    {
        ...
        if (checkAuth(check_api_key())) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $product_id = filter_input_data($this->input->post('p_id', TRUE));
                $per_page = filter_input_data($this->input->post('per_page', TRUE));
                $page = filter_input_data($this->input->post('page', TRUE));
                $user_lang = filter_input_data($this->input->post('language', TRUE));
                if (empty($user_lang)) {
    ...
    */
    public function retrieve_category_product()
    {
        if (checkAuth(check_api_key())) {
            ...
            $this->db->select('a.product_id, a.seller_id, a.category_id, a.price, a.offer_price, a.vat, a.unit, 
                    a.quantity, a.meta_title, a.thumb_image_url,  
                    b.category_name, c.title');
            $this->db->from('product_information a');
            $this->db->join('product_category b','a.category_id = b.category_id','left');
            $this->db->join('product_title c','a.product_id = c.product_id','left');
            $this->db->join('seller_information d','a.seller_id = d.seller_id','left');
    ...
    */
    public function category_name_via_brand()
    {
        if (checkAuth(check_api_key())) {
            ...
            $page = filter_input_data($this->input->post('page', TRUE));
            $user_lang = filter_input_data($this->input->post('language', TRUE));
            $brand = filter_input_data($this->input->post('brand_id', TRUE));
            $all_brand = (explode("--",$brand));
            $lang_id = 0;
            if (empty($user_lang)) {
                $lang_id = 'english';
    ...
    /*
    |---------------------------------------------------
    |   API = product list via brand id 
    |   SQL = SELECT * FROM `product_information` WHERE `brand_id` = 'BY7KJUUG6TS1GNO' ORDER BY `product_info_id` ASC
    |   Result = category_name
    |---------------------------------------------------
    */
    public function product_info_via_brand()
    {
        if (checkAuth(check_api_key())) {
            $this->form_validation->set_rules('brand_id', 'Brand ID', 'trim|required|xss_clean');
            $this->form_validation->set_rules('per_page', 'Data Limit Per Page', 'trim|numeric|is_natural_no_zero|xss_clean');
    ...
    /*
    |---------------------------------------------------
    |   API =api/limarket_api/order_details
